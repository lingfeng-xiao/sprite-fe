...
import { Activity, Battery, Brain, Compass, Loader2, Send, Sparkles } from 'lucide-react'
...
export default function LifePage() {
  ...
  return (
    ...
    >
      ...
      <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-card via-card to-primary/5 shadow-sm">
        ...
        <CardContent>
          <PageSection variant="stats" spacing="compact">
            ...
            <StatCard
              label="Energy"
              value={`${Math.round((lifeSnapshot?.currentState.energyLevel || 0) * 100)}%`}
              icon={Battery}
              size="md"
            />
            <StatCard
          ...
          <CardContent className="space-y-3">
            {lifeSnapshot?.recentChanges.length ? (
              lifeSnapshot.recentChanges.map((change) => (
                <div key={change.changeId} className="rounded-xl border bg-background/70 p-4">
                  <div className="flex items-center justify-between gap-3">
      ...
      {/* Memory Traces + Speak to Sprite Section */}
      <PageSection cols="2">
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-3">
            {lifeSnapshot?.recentMemorySummaries.length ? (
              lifeSnapshot.recentMemorySummaries.map((summary) => (
                <div key={summary} className="rounded-xl border bg-background/70 p-4 text-sm leading-6">
                  {summary}
                </div>
              ))
            ) : (
