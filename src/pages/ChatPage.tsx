...
export default function ChatPage() {
  ...
  return (
    <div className="grid h-full gap-6 xl:grid-cols-[1.2fr_0.8fr]">
      <Card className="flex min-h-[70vh] flex-col overflow-hidden border-primary/10 bg-gradient-to-br from-card to-primary/5">
        ...
        <CardContent className="flex-1 space-y-4 overflow-y-auto">
          {(journal || []).map((entry) => (
            <div key={entry.id} className="rounded-2xl border bg-background/70 p-4 shadow-sm">
          ...
          {latestReply && (
            <div className="rounded-2xl border bg-primary/5 p-4 text-sm leading-6">
              {latestReply}
            </div>
          )}
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="rounded-2xl border bg-background/70 p-4">
              ...
              <div className="mt-2 text-lg font-medium leading-6">{lifeSnapshot?.attentionFocus.description || 'Idle'}</div>
            </div>
            <div className="rounded-2xl border bg-background/70 p-4">
              <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Coherence</div>
              <div className="mt-2 text-lg font-medium">{Math.round((lifeSnapshot?.coherenceScore || 0) * 100)}%</div>
            </div>
          </CardContent>
        </Card>
